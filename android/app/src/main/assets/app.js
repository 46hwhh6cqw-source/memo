...
// ファイル保存。Androidアプリ内では端末に保存し、ブラウザではダウンロードする。
function saveFile(filename, blob) {
  if (window.AndroidBridge && typeof window.AndroidBridge.saveBase64 === "function") {
    ...
    reader.onload = () => {
      ...
      window.AndroidBridge.saveBase64(filename, base64, blob.type || "application/octet-stream");
    };
    reader.onerror = () => alert("保存に失敗しました。");
...
const Memo = (() => {
  ...
  $("#memoDownload").addEventListener("click", () => {
    ...
    if (!n) return;
    flushSave();
    saveFile((n.title || "memo") + ".txt",
      new Blob([n.body], { type: "text/plain;charset=utf-8" }));
  });

  $("#memoExport").addEventListener("click", () => {
    flushSave();
    saveFile(`memo-backup-${new Date().toISOString().slice(0, 10)}.json`,
      new Blob([JSON.stringify({ app: "artmemo", notes }, null, 2)],
...
const Art = (() => {
  ...
  $("#artSaveImg").addEventListener("click", () => {
    ...
    const fname = `art-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.png`;
    c.toBlob((blob) => {
      if (blob) saveFile(fname, blob);
      else alert("画像の書き出しに失敗しました。");
    }, "image/png");
  });
